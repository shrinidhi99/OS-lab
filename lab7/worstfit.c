...
void main()
{
	int max,i,j,k,t,blockSize[100],processSize[100],M,N,PNo[100],BNo[100],isAlloc[100],diff[100],internal,external;
	...
	for(i=0;i<N;i++)
	{
		...
		if(k>0)
		{
			...
			for(t=0;t<M;t++)
			{
				if(blockSize[t]-processSize[i] == diff[max])
				{
					max = t; // here the block number is found
					break;				
				}
			}
			isAlloc[max] = 1;
